...
VkPosColTexPipeline_Ext::VkPosColTexPipeline_Ext(VulkanContext* pVkContext)
{
	const VkDevice device = *pVkContext->GetVkDevice();
	//SHADERS
	auto vertShaderCode = FileUtils::ReadFile("./Shaders/poscoltexVert.spv");
	auto fragShaderCode = FileUtils::ReadFile("./Shaders/poscoltexFrag.spv");
	...
	//Vertexinput
	auto bindingDescription = VertexPosColTex::GetBindingDescription();
	auto attributeDescriptions = VertexPosColTex::GetAttributeDescriptions();
...
std::vector<VkDescriptorSet> VkPosColTexPipeline_Ext::CreateAndWriteDescriptorSets(VkDevice device, VkDescriptorPool descPool, const vector<unique_ptr_del<VkBuffer>>& uniformBuffers, VkImageView texImageView, VkSampler texSampler)
{
...
void VkPosColTexPipeline_Ext::CreateDescriptorSetLayout(VkDevice device)
{
