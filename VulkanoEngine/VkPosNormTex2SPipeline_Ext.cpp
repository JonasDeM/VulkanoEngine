...
VkPosNormTex2SPipeline_Ext::VkPosNormTex2SPipeline_Ext(VulkanContext* pVkContext)
{
	const VkDevice device = *pVkContext->GetVkDevice();

	//SHADERS
	auto vertShaderCode = FileUtils::ReadFile("./Shaders/posnormtexVert.spv");
	auto fragShaderCode = FileUtils::ReadFile("./Shaders/posnormtexFrag.spv");
...
unique_ptr_del<VkDescriptorPool> VkPosNormTex2SPipeline_Ext::CreateDescriptorPool(VkDevice device, const int uboCount)
{
...
void VkPosNormTex2SPipeline_Ext::CreateDescriptorSetLayout(VkDevice device)
{
...
std::vector<VkDescriptorSet> VkPosNormTex2SPipeline_Ext::CreateAndWriteDescriptorSets(VkDevice device, VkDescriptorPool descPool, const vector<unique_ptr_del<VkBuffer>>& uniformBuffers, VkImageView texImageView, VkSampler texSampler)
{
