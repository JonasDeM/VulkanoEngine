...
VkDebugPipeline_Ext::VkDebugPipeline_Ext(VulkanContext* pVkContext)
{
	const VkDevice device = *pVkContext->GetVkDevice();

	//SHADERS
	auto vertShaderCode = FileUtils::ReadFile("./Shaders/debugVert.spv");
	auto fragShaderCode = FileUtils::ReadFile("./Shaders/debugFrag.spv");
	...
	rasterizer.polygonMode = VK_POLYGON_MODE_LINE; //can be wireframe for example, Using any mode other than fill requires enabling a GPU feature.
...
VkDescriptorSet VkDebugPipeline_Ext::CreateAndWriteDescriptorSet(VkDevice device, VkDescriptorPool descPool, VkBuffer uniformBuffer)
{
...
void VkDebugPipeline_Ext::CreateDescriptorSetLayout(VkDevice device)
{
